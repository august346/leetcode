...
class Solution:
    def maxArea(self, height: list[int]) -> int:
        ...
        for ind in s_height:
            ...
            tmp = []
            for another_h, another_ind in [[l, li], [r, ri]]:
                tmp_li, tmp_ri = min(another_ind, ind), max(another_ind, ind)
                tmp_s = min(another_h, s_height[ind]) * (tmp_ri - tmp_li)
                if tmp_s > s:
                    tmp.append([tmp_li, tmp_ri, tmp_s])

            if tmp:
                li, ri, s = max(tmp, key=lambda x: x[2])
                l, r = s_height[li], s_height[ri]
